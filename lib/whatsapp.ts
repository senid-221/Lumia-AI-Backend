...
export async function sendWhatsAppText(phoneNumberId: string, accessToken: string, to: string, body: string) {
  ...
  await db.usageRecord.upsert({ where: { businessId_period: { businessId: account.businessId, period } }, create: { businessId: account.businessId, period, whatsappOut: 1 }, update: { whatsappOut: { increment: 1 } } });
  return data;
}
...
export async function processIncomingWhatsApp(body: any) {
  ...
  let conversation = await db.conversation.findFirst({ where: { businessId: account.businessId, customerId: customer.id, status: 'AI' }, orderBy: { updatedAt: 'desc' } });
  if (!conversation) conversation = await db.conversation.create({ data: { businessId: account.businessId, customerId: customer.id } });
  await db.message.create({ data: { conversationId: conversation.id, role: 'USER', content: text, externalId } });
  const period = new Date().toISOString().slice(0, 7);
  await db.usageRecord.upsert({ where: { businessId_period: { businessId: account.businessId, period } }, create: { businessId: account.businessId, period, messages: 1, whatsappIn: 1 }, update: { messages: { increment: 1 }, whatsappIn: { increment: 1 } } });
